...
def change_state(progress_bar, state_var: tk.IntVar):
    def _inner(*args):
        if state_var.get():
            progress_bar.start()
        else:
...
root.rowconfigure([0, 1, 3, 4, 5], weight=1)
...
for i in range(4):
    ...
    ctk.CTkRadioButton(root, text='Start', variable=state_var, value=1, border_width_checked=4).grid(row=3, column=i,
                                                                                                     padx=(30, 0))
    ctk.CTkRadioButton(root, text='Stop', variable=state_var, value=0, border_width_checked=4).grid(row=4, column=i,
                                                                                                    padx=(30, 0))
    state_var.trace_add('write', change_state(area, state_var))

ctk.CTkLabel(root, text='Thread 5', text_color='gray').grid(row=5, column=0, pady=(0, 10))
ctk.CTkLabel(root, text='Total area: ').grid(row=5, column=1, pady=(0, 10))

sum_area_var = tk.DoubleVar(value=0)
ctk.CTkLabel(root, textvariable=sum_area_var, text_color='red').grid(row=5, column=2, columnspan=2, pady=(0, 10),
                                                                     sticky='w')
